...
export function ChoreographerCalculator() {
  ...
  useEffect(() => {
    if (seniority && workType) {
      if (workType === "project") {
        const selectedData = choreographyProjectSalaryData.find((item) => item.Ansiennitet === Number(seniority))
        if (selectedData && productionLength && rehearsalMonths) {
          const minuteRate = Number(selectedData["Minuttsats koreografi"].replace(/\s/g, ""))
          const monthlyRate = Number(selectedData["Innstudering månedssats"].replace(/\s/g, ""))
          ...
          setSalary({
            ...
            productionSalary: productionSalary.toLocaleString("no-NO", { maximumFractionDigits: 0 }),
            rehearsalSalary: rehearsalSalary.toLocaleString("no-NO", { maximumFractionDigits: 0 }),
        ...
        const selectedData = choreographyTheaterMusicalSalaryData.find((item) => item.Ansiennitet === Number(seniority))
        if (selectedData) {
          setSalary({
