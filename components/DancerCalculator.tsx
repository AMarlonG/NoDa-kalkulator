...
export function DancerCalculator() {
  ...
  const calculateSalary = useCallback(() => {
    if (seniority && employmentType) {
      const selectedData = dancersSalaryData.find((item) => item.Ansiennitet === seniority)
      if (selectedData) {
        if (employmentType === "permanent") {
          ...
          const annualSalary = Number(selectedData["Årslønn prosjekt"].replace(/\s/g, ""))
          ...
          setSalary({
            ...
            totalHourlyFee: totalHourlyFee ? Math.round(totalHourlyFee).toString() : undefined,
          })
  ...
  return (
    <form className="form calculator-content">
      <p className="sr-only">
        Du kan bruke tastaturet for å navigere i kalkulatoren. Bruk tab-tasten for å flytte mellom felt, og enter-tasten
      ...
      <div className="card">
        ...
        <div className="card-content">
          ...
          <div className="card result-card">
            ...
            <div className="card-content result-grid" aria-labelledby="results-title" aria-live="polite">
              {salary ? (
                <div>
                  {employmentType === "permanent" ? (
                    <section className="result-section">
                      <h3 className="result-subtitle">Årslønn (fast ansettelse):</h3>
                      <p className="result-value">
                        {Number(salary.annualSalary.replace(/\s/g, "")).toLocaleString("no-NO", {
                    ...
                    <>
                      <section className="result-section">
                        ...
                        <p className="result-value">
                          {Number(salary.selfEmployedHourlyRate).toLocaleString("no-NO", { maximumFractionDigits: 0 })}{" "}
                          NOK
                        </p>
                    ...
                    <>
                      <section className="result-section">
                        <h3 className="result-subtitle">Årslønn (grunnlag for beregning):</h3>
                        <p className="result-value">
                          {Number(salary.annualSalary.replace(/\s/g, "")).toLocaleString("no-NO", {
                            maximumFractionDigits: 0,
                          })}{" "}
                      ...
                      <section className="result-section">
                        ...
                        <p className="result-value">
                          {Number(salary.dailyRate).toLocaleString("no-NO", { maximumFractionDigits: 0 })} NOK
                        </p>
                        <p className="result-explanation">
                          (Årslønn{" "}
                          {Number(salary.annualSalary.replace(/\s/g, "")).toLocaleString("no-NO", {
                      ...
                      <section className="result-section">
                        <h3 className="result-subtitle">Grunnlønn for prosjektet:</h3>
                        <p className="result-value">
                          {Number(salary.baseSalary).toLocaleString("no-NO", { maximumFractionDigits: 0 })} NOK
                        </p>
                        <p className="result-explanation">
                          (Dagsats {Number(salary.dailyRate).toLocaleString("no-NO", { maximumFractionDigits: 0 })} NOK
                          x {projectDays} dager)
                        </p>
                      </section>
                      <div className="separator"></div>
                      <section className="result-section social-costs">
                        ...
                        <div className="social-costs-list">
                          ...
                          <li className="social-costs-item">
                            Pensjon (2%):
                            <br />
                            {Number(salary.baseSalary).toLocaleString("no-NO", { maximumFractionDigits: 0 })} NOK x 0,02
                            = {Number(salary.socialCosts.pension).toLocaleString("no-NO", { maximumFractionDigits: 0 })}{" "}
                            NOK
                          </li>
                        </div>
                        <p className="result-explanation">(Beregnet basert på grunnlønn for prosjektet)</p>
                      </section>
                      <div className="separator"></div>
                      <section className="result-section total-salary">
                        ...
                        <p className="result-explanation">
                          (Inkluderer grunnlønn for prosjektet og alle sosiale kostnader)
                        </p>
                      </section>
                    </>
                  ) : (
